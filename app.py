...
def predict_and_learn():
    ...
    if 'Date' in web_df.columns:
        ...
        return jsonify({'status': 'Date column is missing in the input data'})
    ...
    for parameter in parameters:
        try:
            ...
            return jsonify({'status': f'Error in SARIMA model fitting for {parameter}: {e}'})
    ...
    for parameter in parameters:
        try:
            ...
            return jsonify({'status': f'Error in SARIMA forecast for {parameter}: {e}'})
    ...
    # Convert forecast_df to dictionary and include in response
    forecast_df_dict = forecast_df.to_dict(orient='records')[0]  # Convert the first row to a dictionary
